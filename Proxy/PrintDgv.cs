...
namespace ProxyClient
{
    class PrintDGV
    {
        ...
        public static void Print_DataGridView(string title, DataGridView dgv1)
        {
            PrintTitle = title;
            ...
            try
            {
                ...
                PrintOptions dlg = new PrintOptions("this is Title", AvailableColumns);
                if (dlg.ShowDialog() != DialogResult.OK) return;
                if (!string.IsNullOrEmpty(dlg.PrintTitle))
                {
                    PrintTitle = dlg.PrintTitle;
                }

                PrintAllRows = dlg.PrintAllRows;
                //FitToPageWidth = dlgFitToPageWidth;
            ...
            try
            {
                ...
                while (RowPos <= dgv.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Draw Header
                            e.Graphics.DrawString(PrintTitle, new Font("微软雅黑", 10, FontStyle.Bold),
                                    Brushes.Black, e.MarginBounds.Left, e.MarginBounds.Top -
                            e.Graphics.MeasureString(PrintTitle, new Font("微软雅黑", 10, FontStyle.Bold),
                                                    e.MarginBounds.Width).Height - 13);
                            String s = DateTime.Now.ToLongDateString() + " " + DateTime.Now.ToShortTimeString();
                            e.Graphics.DrawString(s, new Font("微软雅黑", 10, FontStyle.Bold),
                            ...
                            foreach (DataGridViewColumn GridCol in dgv.Columns)
                            {
                                ...
                                e.Graphics.FillRectangle(new SolidBrush(Color.LightSteelBlue),
                                    new Rectangle((int)ColumnLefts[i], tmpTop + 10,
                                    (int)ColumnWidths[i], HeaderHeight));
                                e.Graphics.DrawRectangle(Pens.Black,
                                    new Rectangle((int)ColumnLefts[i], tmpTop + rowHeight,
                                    (int)ColumnWidths[i], HeaderHeight));
                                e.Graphics.DrawString(GridCol.HeaderText, GridCol.InheritedStyle.Font,
                                    new SolidBrush(GridCol.InheritedStyle.ForeColor),
                                    new RectangleF((int)ColumnLefts[i], tmpTop + rowHeight,
                                    (int)ColumnWidths[i], HeaderHeight), StrFormat);
                                i++;
                            }
                            NewPage = false;
                            tmpTop += HeaderHeight + 5;
                        }
                        ...
                        foreach (DataGridViewCell Cel in GridRow.Cells)
                        {
                                ...
                                ((Type)ColumnTypes[i]).Name == "DataGridViewLinkColumn")
                            {
                                e.Graphics.FillRectangle(new SolidBrush(Color.White),
                                   ...
                                   (int)ColumnWidths[i], (CellHeight + rowHeight)));
                                e.Graphics.DrawRectangle(Pens.LightBlue,
                                   new Rectangle((int)ColumnLefts[i], tmpTop,
                                   (int)ColumnWidths[i], (CellHeight + rowHeight)));
                                e.Graphics.DrawString(Cel.Value.ToSafeString(), Cel.InheritedStyle.Font,
                                        new SolidBrush(Cel.InheritedStyle.ForeColor),
                                        new RectangleF((int)ColumnLefts[i], (float)(tmpTop),
                                        (int)ColumnWidths[i], (float)(CellHeight + rowHeight)), StrFormat);
                            }
                            ...
                            var re = new Rectangle((int)ColumnLefts[i], tmpTop, (int)ColumnWidths[i], CellHeight + 10);

                            // Drawing Cells Borders 
                            e.Graphics.DrawRectangle(new Pen(Color.SteelBlue, 1), re);
                            i++;
                        }
                        tmpTop += CellHeight;
                    }
